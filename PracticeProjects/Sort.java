    ...
    public static String[] mergeInsertSort(String[] Inputarray, String elementorderfilepath, int seperatorval) {
        ...
        while (lastindex < Inputarray.length) {
            ...
            if (chunklist.size() >= 2) {
                ...
                while ((int)chunklist.get(chunklist.size() - 1) == (int)(chunklist.get(chunklist.size() - 2))) {        // Hier ist die besagte while Schleife, Ohne (int) gibt dieses Statement false zurück, sobald die größe der Chunks einen bestimmten Wert überschreitet.
