...
public class Sort {
    ...
    public static String[] MergeSortLinked(String[] Inputarray, String elementorderfilepath) {
        ...
        TLinkedList<String> sortlist = new TLinkedList<String>(Arrays.asList(Inputarray));               //Arraylistsorted on, this algorithm might be slow, but it only uses this Arralist to do all operations on the Stringorder
        ArrayList<Integer> chunklist = new ArrayList<Integer>();                                     // list of chunks sorted
        int[] chunk = new int[]{0,0,0}; 
        Progressbart progressbar = new Progressbart("PracticeProjects/Textfiles/Console.txt", "Mergesort Progress:");
                                                                  // first index of first chunk, first Index of second chunk, index after second chunk
        ...
        while (chunk[2] < Inputarray.length){                                                                                      
            if ((chunklist.size() < 2 || !(chunklist.get(chunklist.size() - 1).equals(chunklist.get(chunklist.size() - 2))))){
                chunklist.add(1);
                chunk[2] +=1;
            }
            if (chunklist.size() > 1) sortlist = mergeLinkedListe(sortlist, counterrefference, true, chunklist, chunk);
            progressbar.update(100.0/(Inputarray.length));
        }
    ...
    /** 
     * merges presorted Array chunks. 
     * @param sortlist      TLinkedList of elements ordered in at least 2 chunks (--> otherwise nothing will really happpen)
     * @param referencemap  A HashMap containing the char --> int value pair to define the order in which characters are supoosed to be sortet
     * @param qualsOnly     if true it merges unequal chunks as well, this will always result in one chunk up to the index of chunk[2] parameter
     * @param chunklist     An Arraylists that has stored each chunk as an int (length of the chunk)
     * @param chunk         The boarders of the last two chunks, {first index first chunk, first Index second chunk, index of last element of chunk 2 + 1} Only have to pass in chunk[3], which has to describe the first index not merged in any chunk so far
     ...
     * @return (not really nessesary) ArrayList with merged Chunks
     */
    public static TLinkedList<String> mergeLinkedListe(TLinkedList<String> sortlist, HashMap<Character, Integer> referencemap, boolean equalsOnly, 
            ArrayList<Integer> chunklist, int[] chunk){
