...
public class MergeSortLinked {
    ...
    public static String[] MergeSortLinkedAlgorythm(String[] Inputarray, String elementorderfilepath) {
        ...
        while (chunk[2] < Inputarray.length) {
            if ((chunklist.size() < 2
                    || !(chunklist.get(chunklist.size() - 1).equals(chunklist.get(chunklist.size() - 2))))) { // add new Chunks of size 1
                chunklist.add(1);
        ...
        if (chunklist.size() > 1)
            sortlist = mergeLinkedListe(sortlist, counterrefference, false, chunklist, chunklistNodes, chunk,
                    chunkNodes);
     ...
     * @return  
     */
    public static TLinkedList<String> mergeSortedArrayListRegionsTLinked(TLinkedList<String> sortliste,
            int IndexBoarder1,
